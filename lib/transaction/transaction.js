...
/**
 * Retrieve a hexa string that can be used with npwd's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more npws than the sum of the input amounts
 * @return {string}
 */
...
Transaction.prototype.checkedSerialize = function(opts) {
  ...
  if (serializationError) {
    serializationError.message += ' - For more information please see: ' +
      'https://npwcore.io/api/lib/transaction#serialization-checks';
 ...
 *
 * Can receive, as output information, the output of npwd's `listunspent` command,
 * and a slightly fancier format recognized by npwcore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a npwcore Address object. The
 * same is true for `script`, which can be a string or a npwcore Script.
 *
 ...
 *
 * // From a pay to public key hash output from npwd's listunspent
...
/**
 * Analogous to npwd's IsCoinBase function in transaction.h
 */
Transaction.prototype.isCoinbase = function() {
  return (this.inputs.length === 1 && this.inputs[0].isNull() && !this.isContainZerocoins());
};

/**
 * Determines if this transaction is stake
 */
Transaction.prototype.isCoinStake = function () {
  return (this.inputs.length > 0 && (!this.inputs[0].isNull()) && this.outputs.length >= 2 && this.outputs[0].isEmpty());
};

/**
 * Determines if this transaction is zerocoin spend
 */
Transaction.prototype.isZerocoinSpend = function () {
  return (this.inputs.length > 0 && this.inputs[0].isZerocoinSpend());
};

/**
 * Determines if this transaction is zerocoin mint
 */
Transaction.prototype.isZerocoinMint = function () {
  for (var i = 0; i < this.outputs.length; i++) {
...
/**
 * Determines if this transaction contain zerocoins
 */
Transaction.prototype.isContainZerocoins = function() {
  return this.isZerocoinMint() || this.isZerocoinSpend();
};
