...
/**
 * A npw transaction script. Each transaction's inputs and outputs
...
/**
 * @returns {boolean} if the script is zerocoin mint
 */
Script.prototype.isZerocoinMint = function() {
  return (this.chunks.length > 0 && this.chunks[0].opcodenum === Opcode.OP_ZEROCOINMINT);
};

/**
 * @returns {boolean} if the script is zerocoin spend
 */
Script.prototype.isZerocoinSpend = function() {
  return (this.chunks.length > 0 && this.chunks[0].opcodenum === Opcode.OP_ZEROCOINSPEND);
};
...
Script.types.ZEROCOINMINT_OUT = 'Zerocoin mint';
Script.types.ZEROCOINSPEND_IN = 'Zerocoin spend';
...
Script.outputIdentifiers.ZEROCOINMINT_OUT = Script.prototype.isZerocoinMint;
...
Script.inputIdentifiers.ZEROCOINSPEND_IN = Script.prototype.isZerocoinSpend;
...
Script.prototype._getOutputAddressInfo = function() {
  ...
  if (this.isScriptHashOut()) {
    ...
    info.hashBuffer = Hash.sha256ripemd160(this.getData());
    info.type = Address.PayToPublicKeyHash;
  } else if (this.isZerocoinMint()) {
    info.hashBuffer = new Buffer('zerocoin');
    info.type = Address.PayToZeroCoin;
  } else {
    return false;
  }
  return info;
...
/**
 * Analogous to npwd's FindAndDelete. Find and delete equivalent chunks,
...
/**
 * Comes from npwd's script interpreter CheckMinimalPush function
...
/**
 * Comes from npwd's script DecodeOP_N function
 * @param {number} opcode
...
/**
 * Comes from npwd's script GetSigOpCount(boolean) function
