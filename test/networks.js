...
describe('Networks', function() {
  ...
  it('will enable/disable regtest Network', function() {
    ...
    networks.testnet.networkMagic.should.deep.equal(new Buffer('a1cf7eac', 'hex'));
    networks.testnet.port.should.equal(61476);
    networks.testnet.dnsSeeds.should.deep.equal([]);
    networks.testnet.regtestEnabled.should.equal(true);

    networks.disableRegtest();
    networks.testnet.networkMagic.should.deep.equal(new Buffer('457667b8', 'hex'));
    networks.testnet.port.should.equal(61474);
    networks.testnet.dnsSeeds.should.deep.equal([
      'test-seed1.npw.live',
      'test-seed2.npw.live',
      'test-seed3.npw.live',
  ...
  it('tests only for the specified key', function() {
    ...
    expect(networks.get(0x8b, 'privatekey')).to.equal(undefined);
  });

  it('can test for multiple keys', function() {
    expect(networks.get(0x8b, ['pubkeyhash', 'scripthash'])).to.equal(networks.testnet);
    expect(networks.get(0x13, ['pubkeyhash', 'scripthash'])).to.equal(networks.testnet);
    expect(networks.get(0x8b, ['privatekey', 'port'])).to.equal(undefined);
  });
